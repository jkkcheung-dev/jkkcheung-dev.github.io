    ...
    experiences: [
        {
            ...
            location: "香港",
            highlights: [
                "約6000名の公的歯科サービス職員が利用する患者記録管理システム（全11モジュール）のうち、3つのモジュール開発に従事しました。主にReactJSによるフロントエンド開発、及びSpring Boot (Java)を用いたバックエンド開発を担当し、RedisやAWSのサービス活用を通じて、ユーザーエクスペリエンスの改善に貢献しました",
                "利用者へのヒアリングを通じて要件の整理・分析を行いました。その上で、システムの過度な複雑化を回避しつつ、ビジネス目標や技術的安定性を考えた改修提案や優先順位の調整を進めました",
                "開発・テスト環境を従来のオンプレミスからAWSのEC2及びEKSへ移行し、高可用性の実現とインフラ管理の手間削減に貢献しました。また、GitHub Actionsを用いたCI/CDパイプラインの構築にも携わり、デプロイの自動化による品質向上とチームの工数削減につなげました",
        ...
        },
        {
            ...
            location: "香港",
            highlights: [
                "社内船積み書類マッピングシステムをレガシーなTibcoソフトウェアから、Spring Bootによる分散サービスへの移行プロジェクトに参画しました。この移行により、チーム間の連携コストの削減や、コンポーネントの独立したスケーラビリティ改善に貢献しました",
                "サービスディスカバリ、サーキットブレーカー、外部設定といったマイクロサービスのベストプラクティスを導入し、社内既存のシステムの一部コンポーネントの置き換えを行いました。CI/CDパイプラインの導入にも携わり、迅速かつ安全なデプロイの構築を通して、システム全体の保守性やパフォーマンス向上に繋げました",
                "チームのトラブルシューティング効率を改善させるため、ログのフレームワークやフォーマットの標準化、およびログの一元管理をチームに提案しました。これにより、運用コストの高い既存ソリューションからの移行を目指しました",
        ...
        },
        {
            ...
            location: "香港",
            highlights: [
                "JavaおよびNode.jsを使用し、顔認証アルゴリズムと連携するRESTful APIサービスの設計・実装に携わりました。顔画像をメタデータと共に安全に登録するテンプレート機能を開発し、セキュアなアクセスとビジネス分析のための顔特徴データ抽出を実現しました。このシステムは1日に1万件以上の顔認証を処理し、オペレーションの効率とセキュリティ強化に貢献しました",
                "ショッピングモールの駐車場アプリと駐車管理システムを連携させるためのRESTful APIサーバー開発を担当しました。APIドキュメントの作成から実装までを行い、オペレーションの効率化と利用者の体験向上に貢献しました",
                "VRフィットネスアプリケーションの開発プロジェクトにおいて、コンセプト設計やハードウェア調査からプロトタイプ制作までを担当しました。Unity3Dエンジンを用い、APIで抵抗・傾斜を制御するスマートバイクトレーナーと360度パノラマ動画を統合し、インタラクティブなフィットネス体験の概念実証（PoC）を行いました"
            ]
        }
    ],
    skills: {
